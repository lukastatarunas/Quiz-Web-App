...
class EditTest extends Component {
  ...
  submit = () => {
    ...
    alert(`Your result successfully submitted in the database!`)
    this.props.onRouteChange(`signIn`)
  }

  addOption = () => {
    this.setState({
      newOption: prompt(`Enter a new option's value:`)
    })
    alert(`New option is successfully added!`)
    this.setState({
  ...
  render() {
    let { selectedOption, index, isEmptyState, previous, next, readyToSubmit, result, totalOptions, newOption } = this.state
    let { id } = this.props
    let optionRows = []
    let data = []
    switch(id) {
    ...
    for (let i = 1; i <= totalOptions; i++) {
        if (i > 5) {
          optionRows.push(<div key={ i }><label><input type='radio' onChange={ this.handleRadioButtonChange } value={ i } checked={ selectedOption === i } ></input><span className='btn btn-outline-dark'> { newOption } </span></label><br></br><br></br></div>)
        }
        if (i <= 5) {
    ...
    return (
      <div className='ma4 mt0'>
        ...
        { previous && <EditTest /> }<br></br>
        { isEmptyState && <Button className='next' value='Next >' onClick={ this.next } />}
        { next && <EditTest /> }<br></br>
        { isEmptyState && <Button value={ `Add Option` } onClick={ this.addOption } /> }<br></br>

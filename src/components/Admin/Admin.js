...
class Admin extends Component {
  ...
  addTest = () => {
    alert(`New test is successfully added!`)
    this.setState({
      totalTests: this.state.totalTests + 1
    })
  }
  ...
  render() {
    ...
    for (let index = 1; index <= totalTests; index++) {
      ...
      tests.push(isEmptyState && <Button key={ index } value={ `Edit Test ${ index }` } onClick={ testName } />)
    }
    return (
      <div>
              { tests }
              { isAddTripState && <EditTest onRouteChange={ onRouteChange } username={ username } key={ key } id={ key } /> }<br></br>
              { viewResults ? <ViewResults /> : null }<br></br>
              { isEmptyState && <Button value={ `+ Add Test` } onClick={ this.addTest } /> }<br></br>
      </div>
    )
  }
}
